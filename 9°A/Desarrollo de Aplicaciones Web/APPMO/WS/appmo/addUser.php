...
$json=array();
	...
	if(isset($_GET["name"]) && isset($_GET["subnamepather"]) && isset($_GET["subnamemother"])
		&& isset($_GET["phone"]) && isset($_GET["type"]) && isset($_GET["addres"])
		...
		$insert="INSERT INTO appmo_user (name, subnamepather, subnamemother, phone, type, addres, numberaddres, colony, cp, city, state)
		VALUES ('{$name}', '{$subnamepather}', '{$subnamemother}','{$phone}', '{$type}', 
		'{$addres}', '{$numberaddres}', '{$colony}', '{$cp}', '{$city}', '{$state}' )";
		...
		if($resultado_insert){
			...
			$resulta["country"]='WS No esta funcionando';
			$json['usuario'][]=$resulta;
			echo json_encode($json);
		}
